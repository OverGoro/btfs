...
struct btfs_symlink_req {
	char target[BTFS_MAX_PATH];
	char linkpath[BTFS_MAX_PATH];
} __packed;

struct btfs_readlink_req {
	char path[BTFS_MAX_PATH];
} __packed;

struct btfs_readlink_resp {
	char target[BTFS_MAX_PATH];
} __packed;
		    ...
		    void *reply, size_t rlen)
{
	...
	ret = wait_event_interruptible_timeout(call->wq, call->done,
					       BTFS_TIMEOUT);
	if (ret <= 0) {
		btfs_call_free(mnt, call);
...
static const struct inode_operations btfs_symlink_iops;
static const struct file_operations btfs_dir_fops;
static const struct file_operations btfs_file_fops;

static struct inode *btfs_new_inode(struct super_block *sb,
				    struct btfs_attr *fa)
{
	...
	} else if (S_ISLNK(fa->mode)) {
		inode->i_op = &btfs_symlink_iops;
	}
			...
			struct kstat *stat, u32 request_mask, unsigned int flags)
{
	...
	ret = btfs_rpc(mnt, BTFS_OP_GETATTR, &req, sizeof(req),
		       &fa, sizeof(fa));
	if (ret)
		return ret;
			...
			struct iattr *attr)
{
	...
	if (attr->ia_valid & ATTR_SIZE) {
		...
		ret = btfs_rpc(mnt, BTFS_OP_TRUNCATE, &req, sizeof(req),
			       NULL, 0);
		if (ret)
			return ret;
	...
	if (attr->ia_valid & ATTR_MODE) {
		...
		req.mode = attr->ia_mode;
		ret = btfs_rpc(mnt, BTFS_OP_CHMOD, &req, sizeof(req), NULL, 0);
		if (ret)
			return ret;
	}
	...
	if (attr->ia_valid & (ATTR_UID | ATTR_GID)) {
		...
		req.uid = from_kuid(&init_user_ns, attr->ia_uid);
		req.gid = from_kgid(&init_user_ns, attr->ia_gid);
		ret = btfs_rpc(mnt, BTFS_OP_CHOWN, &req, sizeof(req), NULL, 0);
		if (ret)
			return ret;
	}
...
static int btfs_symlink(struct mnt_idmap *idmap, struct inode *dir,
			struct dentry *dentry, const char *target)
{
	struct btfs_mnt *mnt = BTFS_MNT(dir->i_sb);
	struct btfs_symlink_req req;
	struct inode *inode;
	int ret;

	memset(&req, 0, sizeof(req));
	strscpy(req.target, target, BTFS_MAX_PATH);

	ret = btfs_path(dentry, req.linkpath, BTFS_MAX_PATH);
	if (ret)
		return ret;

	ret = btfs_rpc(mnt, BTFS_OP_SYMLINK, &req, sizeof(req), NULL, 0);
	if (ret)
		return ret;
...
static const char *btfs_get_link(struct dentry *dentry, struct inode *inode,
				 struct delayed_call *done)
{
	struct btfs_mnt *mnt = BTFS_MNT(inode->i_sb);
	struct btfs_readlink_req req;
	struct btfs_readlink_resp *resp;
	char *link;
	int ret;
	...
	ret = btfs_rpc(mnt, BTFS_OP_READLINK, &req, sizeof(req),
		       resp, sizeof(*resp));
	if (ret) {
		kfree(resp);
		return ERR_PTR(ret);
	...
	link = kstrdup(resp->target, GFP_KERNEL);
	kfree(resp);
...
static const struct inode_operations btfs_dir_iops = {
	...
	.rmdir		= btfs_rmdir,
	.rename		= btfs_rename,
	.symlink	= btfs_symlink,
};

static const struct inode_operations btfs_symlink_iops = {
	.get_link	= btfs_get_link,
	.getattr	= btfs_getattr,
};
...
static int btfs_ensure_open(struct inode *inode, struct file *file)
{
	...
	if (file->f_mode & FMODE_WRITE)
		req.flags = (file->f_mode & FMODE_READ) ? O_RDWR : O_WRONLY;
	else
		req.flags = O_RDONLY;

	ret = btfs_rpc(mnt, BTFS_OP_OPEN, &req, sizeof(req),
		       &rsp, sizeof(rsp));
	if (ret < 0)
		goto err;
...
static ssize_t btfs_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	while (count > 0) {
		...
		ret = btfs_rpc(mnt, BTFS_OP_READ, &req, sizeof(req),
			       rbuf, sizeof(*rsp) + chunk);
		if (ret < 0) {
			kfree(rbuf);
			return total ?: ret;
		}
		...
		if (copy_to_iter(rsp->data, rsp->bytes_read, to) !=
		    rsp->bytes_read) {
			kfree(rbuf);
			return total ?: -EFAULT;
		}
...
static ssize_t btfs_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	while (count > 0) {
		...
		ret = btfs_rpc(mnt, BTFS_OP_WRITE, wreq,
			       sizeof(*wreq) + chunk, &wrsp, sizeof(wrsp));
		kfree(wreq);
...
static int btfs_fsync(struct file *file, loff_t start, loff_t end,
		      int datasync)
{
	struct inode *inode = file_inode(file);
	struct btfs_mnt *mnt = BTFS_MNT(inode->i_sb);
	...
	req.file_handle = bf->fh;
	return btfs_rpc(mnt, BTFS_OP_FSYNC, &req, sizeof(req), NULL, 0);
}
...
static int btfs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	while (off + sizeof(struct btfs_dirent) <= 4096) {
		...
		if (!de->name_len || de->name_len > 255)
			break;

		entry_size = sizeof(*de) + de->name_len + 1;
...
static struct inode *btfs_alloc_inode(struct super_block *sb)
{
	...
	bi = kmem_cache_alloc(btfs_ino_cache, GFP_KERNEL);
	if (!bi)
		return NULL;
...
static int btfs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	struct btfs_mnt *mnt = BTFS_MNT(dentry->d_sb);
	struct {
	...
	ret = btfs_rpc(mnt, BTFS_OP_STATFS, NULL, 0, &resp, sizeof(resp));
	if (ret < 0)
		return ret;

	buf->f_type = BTFS_MAGIC;
	buf->f_bsize = resp.bsize;
	buf->f_blocks = resp.blocks;
	buf->f_bfree = resp.bfree;
	buf->f_bavail = resp.bavail;
	buf->f_files = resp.files;
	buf->f_ffree = resp.ffree;
	buf->f_namelen = resp.namelen;
...
{
	inode_init_once(&((struct btfs_ino *)obj)->vnode);
}
...
static int __init btfs_init(void)
{
	...
	err = register_filesystem(&btfs_type);
	if (err)
		goto err_netlink;
